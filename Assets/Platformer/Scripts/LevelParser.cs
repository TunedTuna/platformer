...
public class LevelParser : MonoBehaviour
{
    ...
    [Header("MISC")]
    public GameObject bg_ref;
    public Material questionSkin;
    ...
    {
        LoadLevel(); 
        bg_ref.gameObject.SetActive(false); //hide ref
        StartCoroutine(UpdateMaterialOffset());//? block change
    }
    ...
    {
        while (true) // Infinite loop to keep it going
        {
            // Assuming the material property has a material with a texture
            // Change the texture offset by 0.2 every second
            questionSkin.mainTextureOffset += new Vector2(0f,0.2f);

            // Wait for 1 second before doing it again
